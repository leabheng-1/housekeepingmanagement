    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Container(
            margin:
                EdgeInsets.only(top: 20), // Set your desired top margin here
            child: Row(
              children: [
                ...
                buildLabelAndContent('Booking Details:', [
                  ...
                  Text(
                      'Housekeeping Status: ${booking['housekeeping_status']}'),
          ...
          actions: [
            Visibility(
              visible: (DateTime.parse(booking['checkin_date'])
                          .isBefore(todaycorrent) ||
                      DateTime.parse(booking['checkin_date'])
                          .isAtSameMomentAs(todaycorrent)) &&
                  booking['booking_status'] !=
                      'In house', // Show "Check In" button if checkInDate is in the future
              child: BtnAction(
                icon: Icons.star,
                textColor: Colors.white,
                color: Colors.blue,
            ...
            Visibility(
              visible: DateTime.parse(booking['checkout_date'])
                  ...
                  DateTime.parse(booking['checkout_date']).isAtSameMomentAs(
                      todaycorrentCheckout), // Show "Check Out" button if checkOutDate is in the past
              child: BtnAction(
                icon: Icons.star,
                textColor: Colors.white,
                color: Colors.blue,
